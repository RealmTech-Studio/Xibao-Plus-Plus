...
public class RenderHelper {
    public static void renderStretchTexture(int width, int height, int light, Identifier source) {
        ...
        bufferBuilder.vertex(0.0D, height, -1.0D).texture(0, 1).color(light, light, light, 255).next();
        bufferBuilder.vertex(width, height, -1.0D).texture(1, 1).color(light, light, light, 255).next();
        bufferBuilder.vertex(width, 0.0D, -1.0D).texture(1, 0).color(light, light, light, 255).next();
        bufferBuilder.vertex(0.0D, 0.0D, -1.0D).texture(0, 0).color(light, light, light, 255).next();
        tessellator.draw();
    }
